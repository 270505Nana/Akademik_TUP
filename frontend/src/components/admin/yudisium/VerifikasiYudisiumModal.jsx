...
const CLR = {
  red: '#C0182A', green: '#16A34A', orange: '#D97706', border: '#E2E8F0', muted: '#94A3B8', text: '#1E293B', sub: '#64748B',
};

const STEPS = [
  { n: 1, label: 'Data Akademik' },
  { n: 2, label: 'Periksa Berkas' },
      ...
      return (
        <React.Fragment key={s.n}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: done ? CLR.green : active ? CLR.red : '#E5E7EB', color: done || active ? '#fff' : '#9CA3AF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, transition: 'all 0.3s ease' }}>
              {done ? <Check size={16} /> : s.n}
            </div>
            <span style={{ fontSize: 11, fontWeight: active ? 700 : 500, color: done ? CLR.green : active ? CLR.red : '#9CA3AF', whiteSpace: 'nowrap' }}>
              {s.label}
            </span>
          </div>
          {i < STEPS.length - 1 && <div style={{ height: 2, width: 80, background: done ? CLR.green : CLR.border, margin: '0 8px', marginBottom: 22, transition: 'background 0.3s ease' }} />}
        </React.Fragment>
      );
    })}
  </div>
...
const InfoCard = ({ label, value, icon: Icon, highlight, valueColor }) => (
  <div style={{ background: '#F8FAFC', border: `1px solid ${CLR.border}`, borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
    ...
    <div>
      <div style={{ fontSize: 10, fontWeight: 700, color: CLR.muted, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 13, fontWeight: 600, color: valueColor || (highlight ? CLR.red : CLR.text), lineHeight: 1.4 }}>{value || '-'}</div>
    </div>
  </div>
);
...
const Step1 = ({ registration, prodiName }) => {
  ...
  const isCumlaude = r.pengajuanCumlaude !== "Non Cumlaude";
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, marginBottom: 20 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: '#B91C1C', textTransform: 'uppercase', letterSpacing: 0.5 }}>Tahap 1: Data Akademik Yudisium</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <InfoCard label="Nama Lengkap" icon={User} value={s.name} />
        <InfoCard label="NIM" icon={Hash} value={s.nim} />
        <InfoCard label="Program Studi" icon={BookOpen} value={prodiName} />
        <InfoCard label="Program" icon={GraduationCap} value={r.program} />
        <InfoCard label="Nilai TAK" icon={CheckCircle2} value={r.tak} />
        <InfoCard label="Skema Sidang" icon={FileText} value={r.skemaSidang} />
        
        {isCumlaude && (
          <div style={{ gridColumn: '1 / -1', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
             <div style={{ width: 36, height: 36, borderRadius: 8, background: '#fff', border: '1px solid #FECACA', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Award size={18} color={CLR.red} />
            </div>
            <div>
               <div style={{ fontSize: 10, fontWeight: 800, color: '#B91C1C', textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 4 }}>Pengajuan Cumlaude</div>
               <div style={{ fontSize: 13, fontWeight: 700, color: '#7F1D1D', marginBottom: 4 }}>{r.pengajuanCumlaude}</div>
               <div style={{ fontSize: 12, fontWeight: 600, color: '#991B1B', marginBottom: 4 }}>Skema: {r.skemaCumlaude || '-'}</div>
               <div style={{ fontSize: 12, color: '#991B1B', background: '#fff', padding: '8px', borderRadius: '6px', border: '1px solid #FECACA', marginTop: '8px', whiteSpace: 'pre-wrap' }}>
                 <strong>Detail Prestasi:</strong><br/>{r.evidenCumlaude || '-'}
               </div>
            </div>
          </div>
        )}
        ...
        {r.berminatWirausaha && (
           <div style={{ gridColumn: '1 / -1' }}>
             <InfoCard label="Minat Wirausaha" icon={Briefcase} value="Mahasiswa memiliki minat untuk berwirausaha" valueColor="#16A34A" />
           </div>
        )}

        <div style={{ gridColumn: '1 / -1' }}>
          <InfoCard label="Judul Tugas Akhir (Indonesia)" icon={FileText} value={r.judulTugasAkhirIndonesia} />
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
          <InfoCard label="Judul Tugas Akhir (Inggris)" icon={FileText} value={r.judulTugasAkhirInggris} />
        </div>
      </div>
    </div>
  );
...
const NavBtn = ({ onClick, disabled, children, title }) => (
  <button onClick={onClick} disabled={disabled} title={title} style={{ width: 24, height: 24, borderRadius: 5, border: `1px solid ${CLR.border}`, background: '#fff', cursor: disabled ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: disabled ? '#D1D5DB' : '#374151', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
    {children}
  </button>
);
...
const Step2 = ({ uploads, berkasStatuses, onToggle, previewFile, onPreview, onDownload, loadingFileId, loadingUploads, fileError }) => {
  ...
  return (
    <div style={{ display: 'flex', gap: 0, flex: 1, minHeight: 0, height: '100%' }}>
      <div style={{ width: 260, flexShrink: 0, borderRight: `1px solid ${CLR.border}`, overflowY: 'auto', padding: '10px 0', height: '100%' }}>
        <div style={{ padding: '0 10px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: CLR.muted, textTransform: 'uppercase', letterSpacing: 0.5 }}>Lampiran Berkas ({uploads.length})</span>
          <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 9999, background: '#DCFCE7', color: CLR.green }}>{Object.values(berkasStatuses).filter(v => v === BERKAS_STATUS.SESUAI).length} Sesuai</span>
        </div>
        {loadingUploads ? (
          ...
          <div style={{ padding: '20px 10px', fontSize: 12, color: '#9CA3AF', textAlign: 'center' }}>Tidak ada berkas diunggah</div>
        ) : null}
        {uploads.map((upload, idx) => {
          ...
          return (
            <div key={upload.id} onClick={() => onPreview(upload)} style={{ padding: '10px', cursor: 'pointer', background: isActive ? '#FEF2F2' : 'transparent', borderLeft: isActive ? `3px solid ${CLR.red}` : '3px solid transparent', transition: 'all 0.15s', borderBottom: '1px solid #F1F5F9' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 7 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#374151', lineHeight: 1.4, marginBottom: 4 }}>{idx + 1}. {berkasName}</div>
                  {status !== BERKAS_STATUS.UNCHECKED && (
                    <div style={{ fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: '4px', display: 'inline-block', background: status === BERKAS_STATUS.SESUAI ? '#DCFCE7' : '#FEE2E2', color: status === BERKAS_STATUS.SESUAI ? CLR.green : '#DC2626' }}>
                      {status === BERKAS_STATUS.SESUAI ? 'SESUAI' : 'BERMASALAH'}
                    </div>
                  )}
                </div>
                <div style={{ flexShrink: 0 }}>
                  {status === BERKAS_STATUS.SESUAI ? <CheckCircle2 size={16} color={CLR.green} /> : status === BERKAS_STATUS.BERMASALAH ? <XCircle size={16} color="#DC2626" /> : <div style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid #D1D5DB', marginTop: '2px' }} />}
                </div>
              </div>
            </div>
          );
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0, height: '100%' }}>
        {previewFile ? (
          <>
            <div style={{ padding: '8px 14px', borderBottom: `1px solid ${CLR.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#FAFAFA', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>
                <FileText size={14} color={CLR.red} flexShrink={0} />
                <span style={{ fontSize: 12, fontWeight: 700, color: '#374151', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{getBerkasName(previewFile)}</span>
              </div>
              <button onClick={() => onDownload(previewFile)} disabled={loadingFileId === previewFile.id} style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 700, background: '#fff', border: `1px solid ${CLR.border}`, cursor: 'pointer', color: '#374151' }}>
                {loadingFileId === previewFile.id ? <Loader size={12} style={{ animation: 'spin 1s linear infinite' }} /> : <Download size={12} />} Unduh
              </button>
            </div>
            <div style={{ flex: 1, minHeight: 0, background: '#F8FAFC', overflow: 'auto', position: 'relative' }}>
              {fileError ? (
                ...
                previewFile.type === 'pdf' ? <iframe src={previewFile.blobUrl} style={{ width: '100%', height: '100%', border: 'none', display: 'block' }} title="Preview" /> : <img src={previewFile.blobUrl} alt="Preview" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain', margin: 'auto', display: 'block', padding: 12 }} />
              ) : (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 8, color: '#9CA3AF' }}><Loader size={18} style={{ animation: 'spin 1s linear infinite' }} /><span style={{ fontSize: 12 }}>Memuat berkas...</span></div>
              )}
            </div>
            <div style={{ flexShrink: 0, padding: '8px 14px', borderTop: `1px solid ${CLR.border}`, background: '#fff', display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 -2px 6px rgba(0,0,0,0.04)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 10, color: '#9CA3AF' }}><FileText size={40} strokeWidth={1.5} /><span style={{ fontSize: 14 }}>Pilih berkas dari daftar kiri untuk memeriksa</span></div>
        )}
      </div>
    </div>
  );
...
const Step3Revisi = ({ berkasStatuses, uploads, dueDate, setDueDate, message, setMessage }) => {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: 10, marginBottom: 20 }}>
        <AlertTriangle size={16} color={CLR.orange} />
        <span style={{ fontSize: 12, fontWeight: 700, color: '#92400E' }}>Ditemukan {bermasalah.length} berkas bermasalah — mahasiswa perlu melakukan perbaikan</span>
      </div>
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: CLR.muted, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>Berkas Perlu Diperbaiki</div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Contoh: Bukti similarity tidak valid, harap upload ulang..."
...
const VerifikasiYudisiumModal = ({ registration, academicStaffId, periodMap, onClose, onSaved }) => {
  ...
  useEffect(() => {
    ...
    const pId = registration.yudisiumPeriodId ?? null;
    if (pId) setSelectedPeriodId(pId);

    const existingUploads = registration.yudisiumRegistrationUploads ?? [];
  ...
  const handlePreview = useCallback(async (upload) => {
    ...
    try {
      const blob = await downloadYudisiumRegistrationUpload(upload.id);
      const isPdf = blob.type.includes('pdf') || (upload.filename || upload.name || '').toLowerCase().endsWith('.pdf');
  ...
  const handleSubmit = async () => {
    ...
    if (hasBermasalah) {
      ...
      if (!selectedPeriodId) { setSubmitError('Pilih periode yudisium terlebih dahulu.'); return; }
    }
    ...
    try {
      if (hasBermasalah) {
        await rejectYudisiumRegistration(registration.id, {
          adminId: academicStaffId,
          message: message.trim(),
          isEdit: `${dueDate}T23:59:59.000Z`,
          yudisiumRegistrationUploadIds: validUploadIds,
        });
      } else {
        await approveYudisiumRegistration(registration.id, {
          adminId: academicStaffId,
          yudisiumPeriodId: selectedPeriodId,
          yudisiumRegistrationUploadIds: validUploadIds,
        });
      }
      onSaved?.();
  ...
  return (
    <div
      style={{
        ...
        left: 0, zIndex: 99999,
        background: 'rgba(15,23,42,0.55)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16,
      }}
      onClick={onClose}
    >
      <motion.div
        initial={{ scale: 0.94, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.94, opacity: 0, y: 20 }}
        transition={{ type: 'spring', damping: 20, stiffness: 300 }} onClick={e => e.stopPropagation()}
        style={{
          background: '#fff', borderRadius: 16, width: '100%', maxWidth: step === 2 ? 1150 : 720, height: '92vh', maxHeight: '92vh',
      ...
      >
        <div style={{ padding: '20px 24px 16px', borderBottom: `1px solid ${CLR.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <h3 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: '#0F172A' }}>Verifikasi Yudisium - {studentName}</h3>
        ...
        <div style={{ flex: 1, overflow: step === 2 ? 'hidden' : 'auto', padding: step === 2 ? 0 : '0 24px 24px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '16px 24px', borderTop: `1px solid ${CLR.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, background: '#FAFAFA' }}>
          <div>
...
export default VerifikasiYudisiumModal;